...
public class CustomerServiceImp implements ICustomerService {
	...
	public List<Customer> getAllCustomer() {
	  ...
	  //Product // http://localhost:8081/api/products/get/{pid}
		  
		  
	Product product = restTemplate.getForObject("http://localhost:8081/api/products/get/"+pid, Product.class);
	...
	@Override
	public String updateProductByCustomerId(int customerId) {
		
		CustomerDTO customer = getCustomerById(customerId);
		
		System.out.println(customer.getProductId());
		Product prod = new Product();
		prod.setPid(customer.getProductId());
		prod.setPrice(95000);
		prod.setPname("ipad");
		
		restTemplate.put("http://localhost:8081/api/products/update",prod);
